...
static void BM_expm_dual7x2(benchmark::State &state) {
  ...
  static_assert(
    std::same_as<math::ElementwiseBinaryOp<
                   math::Array<D, SquareDims<>, true>,
                   math::Array<D, SquareDims<>, true>, std::plus<>>,
                 decltype(A + A)>);
  static_assert(
    std::same_as<double, math::scalarize_via_cast_t<
                           math::Array<D, SquareDims<>, true>>>);
  static_assert(
    std::same_as<double, math::scalarize_via_cast_t<decltype(A + A)>>);
  static_assert(
    std::same_as<
      math::ElementwiseBinaryOp<math::Array<D, SquareDims<>, true>,
  ...
  static_assert(math::ScalarizeViaCastTo<
                math::scalarize_via_cast_t<decltype(view(A))>,
                decltype(2.3 * A)>());
