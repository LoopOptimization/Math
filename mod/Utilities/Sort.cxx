...
using VD = simd::Unroll<1, 8 / W, W, double>;
[[gnu::always_inline]] constexpr void minmax(VD &x, VD &y) {
  VD z = x;
  x = min(x, y);
  y = max(z, y);
}

template <std::ptrdiff_t N, std::ptrdiff_t W>
[[gnu::always_inline]] auto swap_el_pairs(simd::Unroll<1, N, W, double> x)
  -> simd::Unroll<1, N, W, double> {
  static_assert(N * W == 8);
  if constexpr (W == 8) {
    return {__builtin_shufflevector(x.vec_, x.vec_, 2, 3, 0, 1, 6, 7, 4, 5)};
  } else if constexpr (W == 4) {
    simd::Unroll<1, N, W, double> ret;
    simd::Vec<W, double> a = x.data_[0], b = x.data_[1];
    ...
    static_assert(W == 2);
    simd::Unroll<1, N, W, double> ret;
    simd::Vec<W, double> a = x.data_[0], b = x.data_[1], c = x.data_[2],
                         d = x.data_[3];
    ret.data_[0] = b;
    ret.data_[1] = a;
    ret.data_[2] = d;
...
template <std::ptrdiff_t N, std::ptrdiff_t W>
[[gnu::always_inline]] auto reverse2(simd::Unroll<1, N, W, double> x)
  -> simd::Unroll<1, N, W, double> {
  static_assert(N * W == 8);
  if constexpr (W == 8) {
...
template <std::ptrdiff_t N, std::ptrdiff_t W>
[[gnu::always_inline]] auto reverse4(simd::Unroll<1, N, W, double> x)
  -> simd::Unroll<1, N, W, double> {
  static_assert(N * W == 8);
  if constexpr (W == 8) {
...
template <std::ptrdiff_t N, std::ptrdiff_t W>
[[gnu::always_inline]] auto reverse8(simd::Unroll<1, N, W, double> x)
  -> simd::Unroll<1, N, W, double> {
  static_assert(N * W == 8);
  if constexpr (W == 8) {
...
[[gnu::always_inline]] auto vshuf(simd::Unroll<1, N, W, double> x,
                                  simd::Unroll<1, N, W, double> y)
  -> simd::Unroll<1, N, W, double> {
  if constexpr (W == 8) {
    return {__builtin_shufflevector(x.vec_, y.vec_, _0, _1, 8 + _2, 8 + _3,
                                    4 + _0, 4 + _1, 12 + _2, 12 + _3)};
  } else if constexpr (W == 4) {
    simd::Vec<W, double> xa = x.data_[0], xb = x.data_[1], ya = y.data_[0],
                         yb = y.data_[1];
    simd::Unroll<1, N, W, double> ret;
    ...
    static_assert(W == 2);
    simd::Vec<W, double> xa = x.data_[0], xb = x.data_[1], xc = x.data_[2],
...
template <std::ptrdiff_t N, std::ptrdiff_t W>
[[gnu::always_inline]] auto interleave_halves(simd::Unroll<1, N, W, double> x)
  -> simd::Unroll<1, N, W, double> {
  static_assert(N * W == 8);
  if constexpr (W == 8) {
...
template <std::ptrdiff_t N, std::ptrdiff_t W>
[[gnu::always_inline]] auto interleave(simd::Unroll<1, N, W, double> x,
                                       simd::Unroll<1, N, W, double> y)
  -> simd::Unroll<1, N, W, double> {
  static_assert(N * W == 8);
  if constexpr (W == 8) {
...
auto sort(math::SVector<double, 16> x) -> math::SVector<double, 16> {
  ...
  using SV8 = math::SVector<double, 8>;
