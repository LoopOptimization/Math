...
constexpr auto getMaxDigits(const math::Rational *A, std::ptrdiff_t M,
  ...
  -> containers::Flat<int> {
  containers::Flat<int> max_digits(N, 0);
...
constexpr auto getMaxDigits(const T *A, std::ptrdiff_t M, std::ptrdiff_t N,
                            std::ptrdiff_t X) -> containers::Flat<T> {
  containers::Flat<T> max_digits{N, 0};
...
concept ColumnPrintable =
  Printable<T> && (std::same_as<T, double> ||
                   std::is_invocable_r_v<int, detail::DigitsCounter, T>);
