...
constexpr auto vector(alloc::WArena<T> alloc,
                      std::ptrdiff_t M) -> ResizeableView<T, Length<>> {
...
template <class T, std::size_t SlabSize, bool BumpUp>
constexpr auto square_matrix(alloc::Arena<SlabSize, BumpUp> *alloc, std::ptrdiff_t M,
                             T x) -> MutSquarePtrMatrix<T> {
...
template <class T, std::size_t SlabSize, bool BumpUp, std::ptrdiff_t R, std::ptrdiff_t C>
...
matrix(alloc::Arena<SlabSize, BumpUp> *alloc,
       CartesianIndex<std::ptrdiff_t, std::ptrdiff_t> dim) -> MutArray<T, DenseDims<>> {
  Row<> M = Row<>(dim);
  Col<> N = Col<>(dim);
  auto memamt = std::size_t(std::ptrdiff_t(M) * std::ptrdiff_t(N));
  return {alloc->template allocate<compressed_t<T>>(memamt), M, N};
