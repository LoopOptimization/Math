...
template <class T>
constexpr auto vector(alloc::WArena<T> alloc, std::ptrdiff_t M)
  -> ResizeableView<T, Length<>> {
  return {alloc.allocate(M), length(M), capacity(M)};
}
template <class T, std::size_t SlabSize, bool BumpUp>
constexpr auto vector(alloc::Arena<SlabSize, BumpUp> *alloc, std::ptrdiff_t M)
  -> ResizeableView<T, Length<>> {
  return {alloc->template allocate<compressed_t<T>>(M), length(M), capacity(M)};
}

template <class T>
constexpr auto vector(alloc::WArena<T> alloc, std::ptrdiff_t M, T x)
  -> ResizeableView<T, Length<>> {
...
template <class T>
constexpr auto square_matrix(alloc::WArena<T> alloc, std::ptrdiff_t M)
...
constexpr auto square_matrix(alloc::Arena<SlabSize, BumpUp> *alloc,
                             std::ptrdiff_t M) -> MutSquarePtrMatrix<T> {
  return {alloc->template allocate<compressed_t<T>>(M * M),
          SquareDims<>{row(M)}};
}
template <class T>
constexpr auto square_matrix(alloc::WArena<T> alloc, std::ptrdiff_t M, T x)
  -> MutSquarePtrMatrix<T> {
...
template <class T, std::size_t SlabSize, bool BumpUp>
constexpr auto square_matrix(alloc::Arena<SlabSize, BumpUp> *alloc,
                             std::ptrdiff_t M, T x) -> MutSquarePtrMatrix<T> {
...
template <class T, std::ptrdiff_t R, std::ptrdiff_t C>
constexpr auto matrix(alloc::WArena<T> alloc, Row<R> M, Col<C> N)
  -> MutArray<T, DenseDims<R, C>> {
  auto memamt = std::size_t(std::ptrdiff_t(M) * std::ptrdiff_t(N));
  return {alloc.allocate(memamt), DenseDims{M, N}};
}
template <class T, std::size_t SlabSize, bool BumpUp, std::ptrdiff_t R,
          std::ptrdiff_t C>
...
template <class T, std::size_t SlabSize, bool BumpUp>
constexpr auto matrix(alloc::Arena<SlabSize, BumpUp> *alloc,
                      CartesianIndex<std::ptrdiff_t, std::ptrdiff_t> dim)
  -> MutArray<T, DenseDims<>> {
  Row<> M = Row<>(dim);
  Col<> N = Col<>(dim);
  auto memamt = std::size_t(std::ptrdiff_t(M) * std::ptrdiff_t(N));
  return {alloc->template allocate<compressed_t<T>>(memamt), M, N};
...
template <class T, std::ptrdiff_t R, std::ptrdiff_t C>
constexpr auto matrix(alloc::WArena<T> alloc, Row<R> M, Col<C> N, T x)
  -> MutArray<T, DenseDims<R, C>> {
  auto memamt = std::size_t(std::ptrdiff_t(M) * std::ptrdiff_t(N));
...
template <class T>
constexpr auto identity(alloc::WArena<T> alloc, std::ptrdiff_t M)
  -> MutSquarePtrMatrix<T> {
...
template <class T, std::size_t SlabSize, bool BumpUp>
constexpr auto identity(alloc::Arena<SlabSize, BumpUp> *alloc, std::ptrdiff_t M)
  -> MutSquarePtrMatrix<T> {
  MutSquarePtrMatrix<T> A{square_matrix(alloc, M, T{})};
  A.diag() << T{1};
  return A;
