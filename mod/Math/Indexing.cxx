...
private:
  ...
  TRIVIAL friend inline constexpr auto operator+(Begin, std::ptrdiff_t x)
    -> std::ptrdiff_t {
    return x;
  }
  TRIVIAL friend inline constexpr auto operator-(std::ptrdiff_t x, Begin)
...
template <typename T>
concept ScalarIndex =
  std::convertible_to<T, std::ptrdiff_t> || ScalarRelativeIndex<T>;
...
TRIVIAL constexpr auto canonicalizeRange(Colon, std::ptrdiff_t M)
  -> Range<std::ptrdiff_t, std::ptrdiff_t> {
  return Range<std::ptrdiff_t, std::ptrdiff_t>{.b_ = 0, .e_ = M};
}
...
TRIVIAL [[nodiscard]] constexpr auto calcOffset(Length<> len, std::ptrdiff_t i)
  -> std::ptrdiff_t {
  invariant(i >= 0z);
  invariant(i < len);
  return i;
}
TRIVIAL [[nodiscard]] constexpr auto calcOffset(Length<1>, std::ptrdiff_t i)
  -> std::ptrdiff_t {
  invariant(i == 0z);
  return 0z;
}
...
TRIVIAL [[nodiscard]] constexpr auto calcOffset(Length<> len, OffsetEnd i)
  ...
  return std::ptrdiff_t(len) - i.offset_;
}
[[nodiscard]] constexpr auto calcRangeOffset(Length<> len, std::ptrdiff_t i)
  -> std::ptrdiff_t {
  invariant(i <= len);
  return i;
}
...
template <std::ptrdiff_t L = -1, std::ptrdiff_t X = -1> struct StridedRange {
  static constexpr std::ptrdiff_t nrow = L;
...
private:
  TRIVIAL [[nodiscard]] friend inline constexpr auto row(StridedRange r)
    -> Row<> {
    return row(std::ptrdiff_t(r.len_));
  }
...
TRIVIAL inline constexpr auto calcOffset(DenseDims<> len,
                                         simd::index::Unroll<U, W, M> i) {
  if constexpr (std::same_as<M, simd::mask::None<W>>)
    invariant((i.index_ + U * W - 1) < std::ptrdiff_t(len));
  else
...
TRIVIAL constexpr auto calcNewDim(Length<> len,
  ...
  invariant(r.e_ <= len);
  invariant(r.b_ <= r.e_);
  return length(std::ptrdiff_t(r.e_ - r.b_));
}
template <class B, class E>
TRIVIAL constexpr auto calcNewDim(Length<> len, Range<B, E> r) -> Length<> {
  return calcNewDim(len, canonicalizeRange(r, std::ptrdiff_t(len)));
}
...
TRIVIAL constexpr auto calcNewDim(StridedDims<> d, B r, C c) {
  auto rowDims = calcNewDim(length(std::ptrdiff_t(Row(d))), r);
...
TRIVIAL constexpr auto calcNewDim(DenseDims<NR, NC> d, B r, C c) {
  if constexpr (std::same_as<B, Colon>) {
    if constexpr (std::same_as<C, Colon>) {
      ...
      auto col_dims = calcNewDim(length(std::ptrdiff_t(Col(d))), c);
      return StridedDims(row(d), ascol(col_dims), stride(unwrapCol(Col(d))));
    }
  } else if constexpr (std::same_as<C, Colon>) {
    auto row_dims = calcNewDim(length(std::ptrdiff_t(Row(d))), r);
...
TRIVIAL constexpr auto calcNewDim(StridedDims<> d, std::ptrdiff_t,
                                  simd::index::Unroll<C, W, M> c) {
...
template <std::ptrdiff_t U, std::ptrdiff_t W, typename M, std::ptrdiff_t L>
