...
private:
  TRIVIAL friend inline constexpr auto operator+(Range r, std::ptrdiff_t x) {
    return Range{r.b_ + x, r.e_ + x};
  }
  TRIVIAL friend inline constexpr auto operator-(Range r, std::ptrdiff_t x) {
    return Range{r.b_ - x, r.e_ - x};
  }
  TRIVIAL friend inline constexpr auto operator+(std::ptrdiff_t x, Range r) {
    return Range{r.b_ + x, r.e_ + x};
  }
  TRIVIAL friend inline constexpr auto operator-(std::ptrdiff_t x, Range r) {
...
template <typename T> struct StridedIterator {
  ...
  TRIVIAL constexpr auto operator+(std::ptrdiff_t x) const -> StridedIterator {
    return StridedIterator{ptr_ + x * std::ptrdiff_t(stride_), stride_};
  }
  TRIVIAL constexpr auto operator-(std::ptrdiff_t x) const -> StridedIterator {
    return StridedIterator{ptr_ - x * std::ptrdiff_t(stride_), stride_};
  }
  TRIVIAL constexpr auto operator+=(std::ptrdiff_t x) -> StridedIterator & {
    ptr_ += x * std::ptrdiff_t(stride_);
    return *this;
  }
  TRIVIAL constexpr auto operator-=(std::ptrdiff_t x) -> StridedIterator & {
    ptr_ -= x * std::ptrdiff_t(stride_);
  ...
  TRIVIAL constexpr auto operator-(const StridedIterator &other) const
    ...
    return (ptr_ - other.ptr_) / std::ptrdiff_t(stride_);
  }
  TRIVIAL constexpr auto operator+(const StridedIterator &other) const
    -> std::ptrdiff_t {
    invariant(stride_ == other.stride_);
  ...
  TRIVIAL constexpr auto operator[](std::ptrdiff_t x) const -> T & {
    return ptr_[x * std::ptrdiff_t(stride_)];
  }
  TRIVIAL friend constexpr auto operator+(std::ptrdiff_t x,
...
static_assert(std::weakly_incrementable<StridedIterator<std::int64_t>>);
static_assert(std::input_or_output_iterator<StridedIterator<std::int64_t>>);
...
static_assert(std::bidirectional_iterator<StridedIterator<std::int64_t>>,
              "failed bidirectional iterator");

static_assert(std::totally_ordered<StridedIterator<std::int64_t>>,
              "failed random access iterator");
static_assert(std::random_access_iterator<StridedIterator<std::int64_t>>,
              "failed random access iterator");
} // namespace math
