...
constexpr auto gcd(std::int64_t x, std::int64_t y) -> std::int64_t {
  invariant(x != std::numeric_limits<std::int64_t>::min());
  invariant(y != std::numeric_limits<std::int64_t>::min());
  ...
  for (;;) {
    a >>= az;
    std::int64_t d = a - b;
    az = std::countr_zero(std::uint64_t(d));
    b = std::min(a, b);
    if (!d) break;
    a = constexpr_abs(d);
  }
...
constexpr auto gcd(simd::Vec<W, std::int64_t> x, simd::Vec<W, std::int64_t> y)
  -> simd::Vec<W, std::int64_t> {
  constexpr simd::Vec<W, std::int64_t> zero = {};
  constexpr simd::Vec<W, std::int64_t> one = zero + 1;
  constexpr simd::Vec<W, std::int64_t> invalid =
    zero + std::numeric_limits<std::int64_t>::min();
  ...
  b = on ? (b >> bz) : offb;
  simd::Vec<W, std::int64_t> k = on ? (az > bz ? bz : az) : zero;
  for (;;) {
    a >>= az;
    simd::Vec<W, std::int64_t> d = a - b;
    az = simd::crz<W, std::int64_t>(d);
    b = (on & (a <= b)) ? a : b;
    on = on ? (d != zero) : on;
    if (!bool(simd::cmp::ne<W, std::int64_t>(on, zero))) break;
    a = constexpr_abs<W, std::int64_t>(d);
  }
  return b << k;
}
template <>
...
template <std::ptrdiff_t W>
constexpr auto gcdreduce(simd::Vec<W, std::int64_t> v) -> std::int64_t {
  if constexpr (W != 2) {
    simd::Vec<W / 2, std::int64_t> a, b;
    for (std::ptrdiff_t w = 0; w < W / 2; ++w) {
      a[w] = v[w];
      b[w] = v[w + (W / 2)];
    }
...
constexpr auto lcm(std::int64_t x, std::int64_t y) -> std::int64_t {
  ...
  std::int64_t ay = constexpr_abs(y);
  if (ax == 1) return ay;
  if (ay == 1) return ax;
  if (ax == ay) return ax;
