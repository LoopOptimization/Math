...
struct Explicit {
  std::ptrdiff_t capacity_{0z};
  constexpr Explicit() = default;
  constexpr Explicit(std::ptrdiff_t sz)
    : capacity_{((sz > 8z) ? nextpow2(sz) : 8z * (sz > 0))} {}
  constexpr Explicit(::math::Length<> sz) : Explicit(std::ptrdiff_t(sz)) {}
  constexpr auto operator()(auto) const -> std::ptrdiff_t { return capacity_; }
  constexpr auto operator=(std::ptrdiff_t cap) -> Explicit & {
    capacity_ = cap;
    return *this;
  }
  /// args are old nz, new nz
...
protected:
  static constexpr auto totalSizePer() -> std::size_t {
...
struct ManagedSOA : public SOA<T, S, C, TT, II> {
  ...
  ManagedSOA(S nsz) {
    this->sz_ = nsz;
    this->capacity_ = C{std::ptrdiff_t(nsz)};
