...
inline auto printConstraint(std::ostream &os, PtrVector<std::int64_t> a,
  ...
  for (std::ptrdiff_t v = numSyms; v < numVar; ++v) {
...
constexpr void eraseConstraintImpl(MutDensePtrMatrix<std::int64_t> A, Row<> i) {
  std::ptrdiff_t lastRow = std::ptrdiff_t(A.numRow()) - 1;
  invariant(i <= lastRow);
...
constexpr auto substituteEqualityImpl(MutDensePtrMatrix<std::int64_t> E,
...
constexpr auto
substituteEqualityPairImpl(std::array<MutDensePtrMatrix<std::int64_t>, 2> AE,
                           std::ptrdiff_t i) -> Row<> {
  auto [A, E] = AE;
  const auto [numConstraints, numVar] = shape(E);
  std::ptrdiff_t minNonZero = numVar + 1;
...
constexpr auto substituteEquality(MutDensePtrMatrix<std::int64_t> &A,
...
// C = [ I A
//       0 B ]
constexpr void slackEqualityConstraints(MutPtrMatrix<std::int64_t> C,
...
constexpr auto countNonZeroSign(DensePtrMatrix<std::int64_t> A, std::ptrdiff_t i)
  ...
  Row numRow = A.numRow();
  for (std::ptrdiff_t j = 0; j < numRow; ++j) {
...
/// x == 0 -> 0, x < 0 -> 1, x > 0 -> 2
inline constexpr auto orderedCmp(auto x) -> std::ptrdiff_t {
  return (x < 0) | (2 * (x > 0));
}
...
template <bool NonNegative>
constexpr auto fourierMotzkin(Alloc<std::int64_t> auto &alloc,
                              DensePtrMatrix<std::int64_t> A, std::ptrdiff_t v)
...
constexpr void fourierMotzkin(DenseMatrix<std::int64_t> &A, std::ptrdiff_t v) {
...
constexpr void dropEmptyConstraints(MutDensePtrMatrix<std::int64_t> &A) {
...
constexpr auto countSigns(DensePtrMatrix<std::int64_t> A, std::ptrdiff_t i)
  -> std::array<std::ptrdiff_t, 2> {
  std::ptrdiff_t numNeg = 0;
  std::ptrdiff_t numPos = 0;
  for (std::ptrdiff_t j = 0; j < A.numRow(); ++j) {
